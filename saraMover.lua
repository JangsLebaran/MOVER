...
function EventHandler.onNukedWorld(world)
    ...
    print(sformat('[**%s**] %s: %s', bot.world, bot.name, sformat('The world %s is nuked!', world)))
...
function saraNukeChecker.init(config_value)
    ...
    print('NUKE CHECKER SUMMARY')
    for i = 1, #fields[1].value do
        print(fields[1].value[i] .. fields[2].value[i])
    end
end
